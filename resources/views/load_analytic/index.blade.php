            ...
            <div class="flex-none w-2/3 max-w-full px-3">
              <div>
                ...
                <p class="mt-2 font-sans font-semibold leading-normal text-xs dark:text-gray-300">
                  @if(isset($data_loadtest["created_at"]))
                      {{ $data_loadtest["created_at"] }}
                  @else
                      -
                  @endif
                </p>
...
<div class="grid border-black/12.5 shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border dark:bg-gray-800 mb-4 ">
  <div class="flex items-center justify-between">
    ...
    <a id="tombol-download" href="javascript:void(0);">
...
const ldChart4 = new Chart(ctx4, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Web Server with LB (ms)',
            data: dataPengukuran4.map(entry => entry.lb || 0)
        }, {
            label: 'Web Server with No LB (ms)',
...
const ldChart8 = new Chart(ctx8, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Web Server with LB (Kbps)',
            data: data_pengukuran_chart8.map(entry => entry.lb || 0)
        }, {
            label: 'Web Server with No LB (Kbps)',
...
const ldChart9 = new Chart(ctx9, {
    ...
    data: {
        labels: dataChart1.map(entry => entry.x || 0),
        datasets: [{
            label: 'Round Robin (req/sec)',
            data: dataChart1.map(entry => entry.rb_rs || 0)
        }, {
            label: 'Weight Round Robin (req/sec)',
            data: dataChart1.map(entry => entry.wrb_rs || 0)
        }, {
            label: 'Least Connection (req/sec)',
            data: dataChart1.map(entry => entry.lc_rs || 0)
        }, {
            label: 'Weight Least Connection (req/sec)',
...
const ldChart12 = new Chart(ctx12, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Web Server with LB (req/sec)',
            data: data_pengukuran_chart12.map(entry => entry.lb || 0)
        }, {
            label: 'Web Server with No LB (req/sec)',
...
const ldChart17 = new Chart(ctx17, {
    ...
    data: {
        ...
        datasets: [{
            ...
            label: 'Weight Round Robin (s)',
            data: dataChart1.map(entry => entry.wrb_tt || 0)
        }, {
            label: 'Least Connection (s)',
            data: dataChart1.map(entry => entry.lc_tt || 0)
        }, {
            label: 'Weight Least Connection (s)',
...
const ldChart20 = new Chart(ctx20, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Web Server with LB (s)',
            data: data_pengukuran_chart20.map(entry => entry.lb || 0)
        }, {
            label: 'Web Server with No LB (s)',
...
<script>
  document.getElementById('tombol-download').addEventListener('click', function () {
      html2canvas(document.body, {
          onrendered: function (canvas) {
              ...
              pdf.save('download_view_load.pdf');
          }
      });
  });
</script>
