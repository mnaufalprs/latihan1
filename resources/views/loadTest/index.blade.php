                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        @error('server_address')
                            <div id="server_address_error" class="alert alert-danger" style="font-size: 12px; color: red;">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="w-full">
                        
                        <label for="request_count" class="block mb-2 text-sm flex items-center font-medium text-gray-900 dark:text-white">Request Count<button data-popover-target="popover-description2" data-popover-placement="bottom-end" type="button">
    
                            <svg class="w-4 h-4 ms-1 text-gray-400 hover:text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 24"><path fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 13V8m0 8h0m9-4a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                        
                        </button></label>
                        <div data-popover id="popover-description2" role="tooltip" class="absolute z-50 invisible inline-block text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 w-72 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400">
                            <div class="p-3 space-y-2">
                                <p>number of requests at a time and performed simultaneously.</p>
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                        </svg></a>
                            </div>
                            <div data-popper-arrow></div>
                        ...
                        @error('request_count')
                            <div id="request_count_error" class="alert alert-danger" style="font-size: 12px; color: red;">{{ $message }}</div>
                        @enderror
                    </div>

                    <div class="w-full">
                        
                        <label for="connection_count" class="block mb-2 text-sm flex items-center font-medium text-gray-900 dark:text-white">Connection Count<button data-popover-target="popover-description3" data-popover-placement="bottom-end" type="button">
    
                            <svg class="w-4 h-4 ms-1 text-gray-400 hover:text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 24"><path fill-rule="evenodd" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 13V8m0 8h0m9-4a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                        
                        </button></label>
                        <div data-popover id="popover-description3" role="tooltip" class="absolute z-50 invisible inline-block text-sm text-gray-500 transition-opacity duration-300 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 w-72 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400">
                            <div class="p-3 space-y-2">
                                <p>total number of requests to be tested</p>
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                        </svg></a>
                            </div>
                            <div data-popper-arrow></div>
                        ...
                        @error('connection_count')
                            <div id="connection_count_error" class="alert alert-danger" style="font-size: 12px; color: red;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('algortima_LB')
                            <div id="algoritma_error" class="alert alert-danger" style="font-size: 12px; color: red;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('urutan_pengukuran')
                            <div id="urutan_pengukuran_error" class="alert alert-danger" style="font-size: 12px; color: red;">{{ $message }}</div>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ambil pesan kesalahan untuk server_address, connection_count, request_count
    const serverAddressError = document.getElementById('server_address_error');
    const requestCountError = document.getElementById('request_count_error');
    const connectionCountError = document.getElementById('connection_count_error');
    const algortimaError = document.getElementById('algoritma_error');
    const chartError = document.getElementById('chart_error');
    const urutanPengukuranError = document.getElementById('urutan_pengukuran_error');

    // Fungsi untuk mengecek apakah ada pesan kesalahan
    function hasError() {
        return (serverAddressError && serverAddressError.textContent.trim() !== '') ||
               (requestCountError && requestCountError.textContent.trim() !== '') ||
               (connectionCountError && connectionCountError.textContent.trim() !== '') ||
               (algortimaError && algortimaError.textContent.trim() !== '') ||
               (chartError && chartError.textContent.trim() !== '') ||
               (urutanPengukuranError && urutanPengukuranError.textContent.trim() !== '');
    }

    // Atur visibilitas tombol berdasarkan pesan kesalahan
    if (hasError()) {
        // Jika ada pesan kesalahan, tampilkan tombol add testing dan sembunyikan tombol loading
        loadingButton.style.display = 'none';
        stopTestingButton.style.display = 'none';
        addTestingButton.style.display = 'inline-block';
    } else {
        // Jika tidak ada pesan kesalahan, lanjutkan seperti biasa
        const isLoading2 = localStorage.getItem('isLoading2');
    ...
    addTestingButton.addEventListener('click', function() {
        // Ambil pesan kesalahan untuk server_address, connection_count, request_count
        const serverAddressError2 = document.getElementById('server_address_error');
        const requestCountError2 = document.getElementById('request_count_error');
        const connectionCountError2 = document.getElementById('connection_count_error');
        const algortimaError2 = document.getElementById('algoritma_error');
        const chartError2 = document.getElementById('chart_error');
        const urutanPengukuranError2 = document.getElementById('urutan_pengukuran_error');

        // Fungsi untuk mengecek apakah ada pesan kesalahan
        function hasError() {
            return (serverAddressError2 && serverAddressError2.textContent.trim() !== '') ||
                (requestCountError2 && requestCountError2.textContent.trim() !== '') ||
                (connectionCountError2 && connectionCountError2.textContent.trim() !== '') ||
                (algortimaError2 && algortimaError2.textContent.trim() !== '') ||
                (chartError2 && chartError2.textContent.trim() !== '') ||
                (urutanPengukuranError2 && urutanPengukuranError2.textContent.trim() !== '');
        }

        if(hasError()){
            localStorage.setItem('isLoading2', 'false');
        } else {
            // Ketika tombol add testing diklik, set status loading ke true
            localStorage.setItem('isLoading2', 'true');
